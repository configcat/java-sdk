...
public class CacheUtilTests {
    ...
    public void serialize() {
        ...
        String fetchTimeRaw = CacheUtils.DateTimeUtils.format(System.currentTimeMillis());
        Entry entry = new Entry(config, "fakeTag", json, fetchTimeRaw);

        String serializedString = CacheUtils.serialize(entry);
    ...
    public void deserialize() throws Exception {
        ...
        String fetchTimeRaw = CacheUtils.DateTimeUtils.format(currentTimeMillis);

        Entry entry = CacheUtils.deserialize(String.format(SERIALIZED_DATA, fetchTimeRaw, "fakeTag", json));
    ...
    public void deserializeMissingValue() throws Exception {
        Entry deserializeNull = CacheUtils.deserialize(null);
        assertTrue(deserializeNull.isEmpty());
        Entry deserializeEmpty = CacheUtils.deserialize("");
        assertTrue(deserializeEmpty.isEmpty());
    }

    @Test
    public void deserializeWrongFormat() throws Exception {
        Exception assertThrows = assertThrows(Exception.class, () -> CacheUtils.deserialize("value with no new line"));
        assertEquals("Number of values is fewer than expected.", assertThrows.getMessage());

        assertThrows = assertThrows(Exception.class, () -> CacheUtils.deserialize("value with one \n new line"));
        assertEquals("Number of values is fewer than expected.", assertThrows.getMessage());
    }

    @Test
    public void deserializeInvalidDate() throws Exception {
        Exception assertThrows = assertThrows(Exception.class, () -> CacheUtils.deserialize(String.format(SERIALIZED_DATA, "invalid", "fakeTag", "json")));
    ...
    public void deserializeInvalidETag() {
        long currentTimeMillis = System.currentTimeMillis();
        String fetchTimeRaw = CacheUtils.DateTimeUtils.format(currentTimeMillis);
        Exception assertThrows = assertThrows(Exception.class, () -> CacheUtils.deserialize(String.format(SERIALIZED_DATA, fetchTimeRaw, "", "json")));
    ...
    public void deserializeInvalidJson() {
        long currentTimeMillis = System.currentTimeMillis();
        String fetchTimeRaw = CacheUtils.DateTimeUtils.format(currentTimeMillis);
        Exception assertThrows = assertThrows(Exception.class, () -> CacheUtils.deserialize(String.format(SERIALIZED_DATA, fetchTimeRaw, "fakeTag", "")));

        assertEquals("Empty config jsom value.", assertThrows.getMessage());

        assertThrows = assertThrows(Exception.class, () -> CacheUtils.deserialize(String.format(SERIALIZED_DATA, fetchTimeRaw, "fakeTag", "wrongjson")));
