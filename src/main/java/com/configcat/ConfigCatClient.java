...
public final class ConfigCatClient implements ConfigurationProvider {
    ...
    public <T> T getValue(Class<T> classOfT, String key, User user, T defaultValue) {
        ...
        try {
            ...
            EvaluationDetails<Object> evaluationDetails = EvaluationDetails.fromError(key, defaultValue,
                    EvaluationErrorCode.UNEXPECTED_ERROR, e.getMessage(), e, effectiveUser);
            ...
            EvaluationDetails<Object> evaluationDetails = EvaluationDetails.fromError(key, defaultValue,
                    EvaluationErrorCode.fromException(e), e.getMessage(), e, effectiveUser);
            this.configCatHooks.invokeOnFlagEvaluated(evaluationDetails);
            return defaultValue;
        }
    }
    ...
    public <T> EvaluationDetails<T> getValueDetails(Class<T> classOfT, String key, User user, T defaultValue) {
        ...
        try {
            ...
            Thread.currentThread().interrupt();
            EvaluationDetails<Object> evaluationDetails = EvaluationDetails.fromError(key, defaultValue,
                    EvaluationErrorCode.UNEXPECTED_ERROR, error + ": " + e.getMessage(), e, effectiveUser);
            this.configCatHooks.invokeOnFlagEvaluated(evaluationDetails);
                ...
                .thenApply(settingsResult -> {
                    try {
                        ...
                        if (checkSettingResult.error() != null) {
                            EvaluationDetails<Object> evaluationDetails = EvaluationDetails.fromError(key, defaultValue,
                                    checkSettingResult.errorCode(), checkSettingResult.error(), null, effectiveUser);
                            this.configCatHooks.invokeOnFlagEvaluated(evaluationDetails);
                            return evaluationDetails.asTypeSpecific();
                        }
                    ...
                    try {
                        ...
                        for (String key : keys) {
                            ...
                            Object value = this.evaluateObject(this.classBySettingType(setting.getType()), setting, key,
                                    effectiveUser, settingResult.fetchTime(), settings).getValue();
    ...
    private EvaluationDetails<Object> evaluateObject(Class<?> classOfT, Setting setting, String key, User effectiveUser, Long fetchTime, Map<String, Setting> settings) {
    ...
    private <T> EvaluationDetails<T> evaluate(Class<T> classOfT, Setting setting, String key, User effectiveUser, Long fetchTime, Map<String, Setting> settings) {
        return evaluateObject(classOfT, setting, key, effectiveUser, fetchTime, settings).asTypeSpecific();
    }
